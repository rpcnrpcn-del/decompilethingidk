...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	private bool CallAuthenticate()
	{
		...
		if (authenticationValues == null)
		{
			...
			authenticationValues2.UserId = null;
	...
	private void RebuildPlayerListCopies()
	{
		...
		Array.Sort(mPlayerListCopy, (PhotonPlayer x, PhotonPlayer y) => x.ID - y.ID);
		List<PhotonPlayer> list = new List<PhotonPlayer>();
		for (int num = 0; num < mPlayerListCopy.Length; num++)
		{
			PhotonPlayer photonPlayer = mPlayerListCopy[num];
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		else if (operationResponse.OperationCode == 226 && operationResponse.ReturnCode == 32758)
		{
			Debug.LogWarning("Operation failed: " + operationResponse.ToStringFull());
		}
		else if (operationResponse.OperationCode == 252 && operationResponse.ReturnCode == -2)
		{
			Debug.LogWarning("Operation failed: " + operationResponse.ToStringFull());
		}
		else
	...
	{
		PhotonNetwork.LockSendMonoMessageTargets();
		try
		{
		...
		{
			PhotonNetwork.UnlockMonoMessageTargets();
		}
	}
	...
	public void NewSceneLoaded()
	{
		...
		if (PhotonNetwork.loadLevelStartTime.HasValue)
		{
			AnalyticsHelper.SceneLoadDuration(Time.realtimeSinceStartup - PhotonNetwork.loadLevelStartTime.Value);
			PhotonNetwork.loadLevelStartTime = null;
		}
		List<int> list = new List<int>();
