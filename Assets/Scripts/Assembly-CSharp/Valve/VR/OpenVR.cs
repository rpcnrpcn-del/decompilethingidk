...
namespace Valve.VR
{
	public class OpenVR
	{
		private class COpenVRContext
		{
			...
			public CVRSystem VRSystem()
			{
				...
				if (m_pVRSystem == null)
				{
					...
					IntPtr genericInterface = OpenVRInterop.GetGenericInterface("FnTable:IVRSystem_012", ref peError);
			...
			public CVRCompositor VRCompositor()
			{
				...
				{
					EVRInitError peError = EVRInitError.None;
					IntPtr genericInterface = OpenVRInterop.GetGenericInterface("FnTable:IVRCompositor_016", ref peError);
					if (genericInterface != IntPtr.Zero && peError == EVRInitError.None)
					{
			...
			public CVROverlay VROverlay()
			{
				...
				{
					EVRInitError peError = EVRInitError.None;
					IntPtr genericInterface = OpenVRInterop.GetGenericInterface("FnTable:IVROverlay_013", ref peError);
					if (genericInterface != IntPtr.Zero && peError == EVRInitError.None)
					{
		...
		public static CVRSystem Init(ref EVRInitError peError, EVRApplicationType eApplicationType = EVRApplicationType.VRApplication_Scene)
		{
			...
			if (!IsInterfaceVersionValid("IVRSystem_012"))
			{
				ShutdownInternal();
