...
public class PhotonView : MonoBehaviour
{
	...
	public int ownerId
	{
		...
		set
		{
			...
			{
				_ownerId = value;
				if (this.OwnerChangedEvent != null)
				{
					this.OwnerChangedEvent(_ownerId);
				}
			}
		}
	}
	...
	public bool isMineNoMaster
	{
		get
		{
			return ownerId == PhotonNetwork.player.ID;
		}
	}
	...
	public event Action<int> OwnerChangedEvent;

	private void InitIsLocal()
	{
		if (!_isLocal.HasValue)
		{
			_isLocal = CreatorActorNr == PhotonNetwork.player.ID;
		}
	}
	...
	protected internal void Awake()
	{
		...
		{
			InitIsLocal();
			_ownerId = viewID / PhotonNetwork.MAX_VIEW_IDS;
			PhotonNetwork.networkingPeer.RegisterPhotonView(this);
			instantiationDataField = PhotonNetwork.networkingPeer.FetchInstantiationData(instantiationId);
		}
		didAwake = true;
	...
	public void OnRPCReceived(PhotonPlayer sender, string methodName)
	{
	}

	public void OnRPCInvoked(string methodName)
	{
	}
