...
public static class PhotonNetwork
{
	...
	private static int SendMonoMessageLock;

	private static HashSet<GameObject> QueuedAddSendMonoMessageTargets;

	private static HashSet<GameObject> QueuedRemoveSendMonoMessageTargets;
	...
	public static HashSet<GameObject> SendMonoMessageTargets { get; private set; }
	...
	public static long NetworkStatisticsBytesOut
	{
		get
		{
			return (networkingPeer != null && !offlineMode) ? networkingPeer.BytesOut : 0;
		}
	}

	public static long NetworkStatisticsBytesIn
	{
		get
		{
			return (networkingPeer != null && !offlineMode) ? networkingPeer.BytesIn : 0;
		}
	}

	public static float? loadLevelStartTime { get; set; }
	...
	static PhotonNetwork()
	{
		...
		SendMonoMessageLock = 0;
		QueuedAddSendMonoMessageTargets = new HashSet<GameObject>();
		QueuedRemoveSendMonoMessageTargets = new HashSet<GameObject>();
	...
	private static bool VerifyCanUseNetwork()
	{
		...
		UnityEngine.Debug.LogWarning("Cannot send messages when not connected. Either connect to Photon OR use offline mode!");
		return false;
	}
	...
	public static void LockSendMonoMessageTargets()
	{
		SendMonoMessageLock++;
	}

	public static void UnlockMonoMessageTargets()
	{
		SendMonoMessageLock--;
		if (SendMonoMessageLock != 0 || SendMonoMessageTargets == null)
		{
			return;
		}
		foreach (GameObject queuedRemoveSendMonoMessageTarget in QueuedRemoveSendMonoMessageTargets)
		{
			RemoveMonoMessageTarget(queuedRemoveSendMonoMessageTarget);
		}
		foreach (GameObject queuedAddSendMonoMessageTarget in QueuedAddSendMonoMessageTargets)
		{
			AddMonoMessageTarget(queuedAddSendMonoMessageTarget);
		}
	}

	public static void AddMonoMessageTarget(GameObject gameObject)
	{
		if (SendMonoMessageTargets == null)
		{
			SendMonoMessageTargets = new HashSet<GameObject>();
		}
		if (SendMonoMessageLock > 0)
		{
			QueuedRemoveSendMonoMessageTargets.Remove(gameObject);
			QueuedAddSendMonoMessageTargets.Add(gameObject);
		}
		else
		{
			SendMonoMessageTargets.Add(gameObject);
		}
	}

	public static void RemoveMonoMessageTarget(GameObject gameObject)
	{
		if (SendMonoMessageLock > 0)
		{
			QueuedRemoveSendMonoMessageTargets.Add(gameObject);
			QueuedAddSendMonoMessageTargets.Remove(gameObject);
		}
		else if (SendMonoMessageTargets != null)
		{
			SendMonoMessageTargets.Remove(gameObject);
		}
	}
	...
	public static void LoadLevel(int levelNumber)
	{
		...
		loadLevelStartTime = Time.realtimeSinceStartup;
