...
public class SteamVR_TrackedCamera
{
	public class VideoStreamTexture
	{
		...
		private void Update()
		{
			...
			else if (trackedCamera.GetVideoStreamTextureD3D11(videostream.handle, frameType, texture2D.GetNativeTexturePtr(), ref ppD3D11ShaderResourceView, ref header, nFrameHeaderSize) != EVRTrackedCameraError.None)
			{
				return;
			}
			if (_texture == null)
