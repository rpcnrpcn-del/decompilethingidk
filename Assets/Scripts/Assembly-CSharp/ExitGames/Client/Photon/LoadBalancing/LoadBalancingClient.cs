...
namespace ExitGames.Client.Photon.LoadBalancing
{
	public class LoadBalancingClient : IPhotonPeerListener
	{
		...
		public bool IsConnected
		{
			get
			{
				return loadBalancingPeer != null && State != ClientState.Uninitialized && State != ClientState.Disconnected;
			}
		}
		...
		{
			return OpSetCustomPropertiesOfActor(actorNr, propertiesToSet, expectedProperties, (!webForward) ? null : new WebFlags(1));
		}
		...
		{
			return OpSetCustomPropertiesOfRoom(propertiesToSet, expectedProperties, (!webForward) ? null : new WebFlags(1));
		}
		...
		private void SetupEncryption(Dictionary<byte, object> encryptionData)
		{
			...
			case EncryptionMode.DatagramEncryption:
			{
				...
				loadBalancingPeer.InitDatagramEncryption(array, hmacSecret);
				break;
			}
			default:
				throw new ArgumentOutOfRangeException();
		...
		public virtual void OnEvent(EventData photonEvent)
		{
			...
			case byte.MaxValue:
			{
				...
				Player player = CurrentRoom.GetPlayer(num3);
				if (player == null)
				{
					Player player2 = CreatePlayer(string.Empty, num3, false, hashtable3);
					CurrentRoom.StorePlayer(player2);
				}
				else
				{
			...
			case 254:
			{
				...
				if (photonEvent.Parameters.ContainsKey(203))
				{
					...
					if (num2 != 0)
					{
						CurrentRoom.masterClientIdField = num2;
					}
				}
				break;
			}
