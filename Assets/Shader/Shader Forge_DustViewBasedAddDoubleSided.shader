Shader "Shader Forge/DustViewBasedAddDoubleSided" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp0;
                float4 tmp1;
                tmp0.x = inp.texcoord2.z - _ProjectionParams.y;
                tmp0.x = max(tmp0.x, 0.0);
                tmp0.yz = inp.texcoord.xy * _Texture_ST.xy + _Texture_ST.zw;
                tmp1 = tex2D(_Texture, tmp0.yz);
                tmp0.yzw = tmp1.xyz * inp.color.www;
                o.sv_target.w = tmp0.x * tmp0.y;
                tmp0.xyz = tmp0.yzw * _Color.xyz;
                o.sv_target.xyz = tmp0.xyz * _Color.www;
                return o;
			}
			ENDCG
		}
