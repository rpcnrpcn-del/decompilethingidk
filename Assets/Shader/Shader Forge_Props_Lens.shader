	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5.xyz = tmp6.xyz + inp.texcoord8.xyz;
                ...
                tmp2.xy = float2(1.0, 1.00001) - tmp0.yy;
                tmp0.z = log(tmp2.x);
                tmp0.z = tmp0.z * _Fresnel;
                tmp0.z = exp(tmp0.z);
                tmp2.z = dot(tmp4.xyz, tmp1.xyz);
                tmp2.z = max(tmp2.z, 0.0);
                tmp4.xy = inp.texcoord.xy * _Specular_ST.xy + _Specular_ST.zw;
                tmp4 = tex2D(_Specular, tmp4.xy);
                tmp3.w = max(tmp4.y, tmp4.x);
                tmp3.w = max(tmp4.z, tmp3.w);
                tmp1.x = dot(tmp3.xyz, tmp1.xyz);
                tmp1.y = tmp0.x * tmp7.y + tmp2.w;
                tmp0.y = tmp0.y * tmp7.y + tmp2.w;
                tmp0.y = tmp1.y * tmp0.y + 0.00001;
                tmp0.y = 1.0 / tmp0.y;
                tmp0.y = tmp0.y * 0.25;
                tmp1.y = tmp1.w * tmp1.w;
                tmp1.y = tmp1.y * tmp1.y;
                tmp1.xy = max(tmp1.xy, float2(0.0, 0.0001));
                tmp1.y = 2.0 / tmp1.y;
                tmp1.y = tmp1.y - 2.0;
                ...
                tmp0.y = saturate(tmp3.w + _Gloss);
                tmp6.xyz = tmp0.zzz * tmp7.xzw;
                tmp7.xy = tmp2.xy * tmp2.xy;
                tmp0.w = tmp2.x * tmp7.x;
                tmp0.w = tmp2.x * tmp0.w;
                tmp0.w = tmp2.x * tmp0.w;
                tmp7.xzw = tmp0.yyy - tmp4.xyz;
                tmp4.xyz = tmp0.www * tmp7.xzw + tmp4.xyz;
                tmp4.xyz = tmp4.xyz * tmp6.xyz;
                tmp1.xyz = tmp1.xyz * tmp3.xyz + tmp4.xyz;
                tmp0.y = tmp2.z + tmp2.z;
                tmp0.y = tmp2.z * tmp0.y;
                ...
                tmp0.xzw = tmp0.xxx * _LightColor0.xyz + tmp5.xyz;
                o.sv_target.xyz = tmp0.xzw * tmp0.yyy + tmp1.xyz;
                o.sv_target.w = 1.0;
                return o;
			}
			ENDCG
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.w = tmp0.x * tmp1.w + tmp1.z;
                tmp0.w = tmp0.w * tmp2.x + 0.00001;
                tmp0.w = 1.0 / tmp0.w;
                tmp0.w = tmp0.w * 0.25;
                ...
                tmp1.y = _Gloss * 10.0 + 1.0;
                tmp1.y = exp(tmp1.y);
                tmp0.z = tmp0.z * tmp1.y;
                tmp0.z = exp(tmp0.z);
                tmp1.y = dot(inp.texcoord7.xyz, inp.texcoord7.xyz);
                tmp3 = tex2D(_LightTexture0, tmp1.yy);
                tmp1.yzw = tmp3.xxx * _LightColor0.xyz;
                tmp3.xyz = tmp0.zzz * tmp1.yzw;
                ...
                tmp2.xw = inp.texcoord.xy * _Specular_ST.xy + _Specular_ST.zw;
                tmp4 = tex2D(_Specular, tmp2.xw);
                ...
                tmp1.x = 1.0 - tmp1.x;
                tmp0 = tmp0.xxxw * tmp1.yzwx;
                o.sv_target.xyz = tmp0.xyz * tmp0.www + tmp3.xyz;
                o.sv_target.w = 0.0;
                return o;
			}
			ENDCG
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = exp(tmp0.x);
                tmp0.yz = inp.texcoord.xy * _Specular_ST.xy + _Specular_ST.zw;
                tmp1 = tex2D(_Specular, tmp0.yz);
