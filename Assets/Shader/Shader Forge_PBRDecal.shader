	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp3.xy = inp.texcoord.xy * _NormalTex_ST.xy + _NormalTex_ST.zw;
                tmp3 = tex2D(_NormalTex, tmp3.xy);
                tmp3.xy = tmp3.wy * float2(2.0, 2.0) + float2(-1.0, -1.0);
                tmp1.w = dot(tmp3.xy, tmp3.xy);
                tmp1.w = min(tmp1.w, 1.0);
                tmp1.w = 1.0 - tmp1.w;
                tmp1.w = sqrt(tmp1.w);
                tmp3.xy = tmp3.xy * _Normal.xx;
                ...
                if (tmp2.w) {
                    ...
                    tmp5.xyz = tmp2.www ? tmp5.xyz : inp.texcoord3.xyz;
                    ...
                    tmp2.w = max(tmp2.w, tmp4.w);
                    tmp5.x = min(tmp5.y, tmp2.w);
                ...
                if (tmp5.w) {
                    ...
                    if (tmp5.w) {
                        ...
                        tmp0.xyz = tmp8.xyz * tmp5.www + tmp9.xyz;
                    }
                    tmp8 = UNITY_SAMPLE_TEXCUBE_SAMPLER(unity_SpecCube0, unity_SpecCube0, float4(tmp0.xyz, tmp3.w));
                    ...
                    tmp6.xyz = tmp4.www * tmp6.xyz + -tmp0.xyz;
                    tmp7.xzw = unity_SpecCube0_BoxMin.www * tmp6.xyz + tmp0.xyz;
                }
                tmp0.x = dot(tmp3.xyz, tmp4.xyz);
                tmp0.y = dot(tmp4.xyz, tmp1.xyz);
                tmp0.xy = max(tmp0.xy, float2(0.0, 0.0));
                tmp4.xy = inp.texcoord.xy * _SpecTex_ST.xy + _SpecTex_ST.zw;
                tmp4 = tex2D(_SpecTex, tmp4.xy);
                tmp0.z = max(tmp4.y, tmp4.x);
                tmp0.z = max(tmp4.z, tmp0.z);
                tmp2.x = dot(tmp3.xyz, tmp2.xyz);
                tmp2.x = max(tmp2.x, 0.0);
                tmp1.x = dot(tmp3.xyz, tmp1.xyz);
                tmp1.y = tmp0.x * tmp7.y + tmp2.w;
                tmp1.z = tmp2.x * tmp7.y + tmp2.w;
                ...
                tmp2.yzw = float3(1.0, 1.0, 1.0) - tmp4.xyz;
                tmp2.yzw = tmp2.yzw * tmp0.www + tmp4.xyz;
                tmp0.w = saturate(tmp0.z + _Gloss);
                tmp3.xy = float2(1.0, 1.00001) - tmp2.xx;
                tmp3.zw = tmp3.xy * tmp3.xy;
                tmp3.zw = tmp3.zw * tmp3.zw;
                tmp3.xy = tmp3.xy * tmp3.zw;
                tmp6.xyz = tmp0.www - tmp4.xyz;
                tmp3.xzw = tmp3.xxx * tmp6.xyz + tmp4.xyz;
                tmp3.xzw = tmp3.xzw * tmp7.xzw;
                tmp1.xyz = tmp1.xyz * tmp2.yzw + tmp3.xzw;
                ...
                tmp0.w = tmp0.y * tmp0.w + 1.0;
                tmp0.y = tmp0.y * tmp3.y + 1.0;
                tmp0.y = tmp0.y * tmp0.w;
                tmp0.x = tmp0.x * tmp0.y;
                tmp0.yw = inp.texcoord.xy * _MainTex_ST.xy + _MainTex_ST.zw;
                tmp2 = tex2D(_MainTex, tmp0.yw);
                tmp0.yw = inp.texcoord1.xy * _DecalTex_ST.xy + _DecalTex_ST.zw;
                tmp3 = tex2D(_DecalTex, tmp0.yw);
                tmp3.xyz = tmp3.xyz - tmp2.xyz;
                tmp2.xyz = tmp3.www * tmp3.xyz + tmp2.xyz;
                tmp2.xyz = tmp2.xyz * _Color.xyz;
                tmp0.y = 1.0 - tmp0.z;
                tmp0.yzw = tmp0.yyy * tmp2.xyz;
                tmp2.xyz = tmp0.xxx * _LightColor0.xyz + tmp5.xyz;
                tmp0.xyz = tmp2.xyz * tmp0.yzw + tmp1.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = rsqrt(tmp0.x);
                tmp0.xyz = tmp0.xxx * inp.texcoord4.xyz;
                tmp1.xy = inp.texcoord.xy * _NormalTex_ST.xy + _NormalTex_ST.zw;
                tmp1 = tex2D(_NormalTex, tmp1.xy);
                tmp1.xy = tmp1.wy * float2(2.0, 2.0) + float2(-1.0, -1.0);
                tmp0.w = dot(tmp1.xy, tmp1.xy);
                tmp1.xy = tmp1.xy * _Normal.xx;
                tmp0.w = min(tmp0.w, 1.0);
                tmp0.w = 1.0 - tmp0.w;
                tmp0.w = sqrt(tmp0.w);
                tmp1.yzw = tmp1.yyy * inp.texcoord6.xyz;
                tmp1.xyz = tmp1.xxx * inp.texcoord5.xyz + tmp1.yzw;
                ...
                tmp3.xy = inp.texcoord.xy * _SpecTex_ST.xy + _SpecTex_ST.zw;
                tmp3 = tex2D(_SpecTex, tmp3.xy);
                ...
                tmp3.xy = inp.texcoord1.xy * _DecalTex_ST.xy + _DecalTex_ST.zw;
                tmp3 = tex2D(_DecalTex, tmp3.xy);
                tmp3.xyz = tmp3.xyz - tmp2.xyz;
                tmp2.xyz = tmp3.www * tmp3.xyz + tmp2.xyz;
                tmp2.xyz = tmp2.xyz * _Color.xyz;
                tmp2.xyz = tmp0.www * tmp2.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.xy = inp.texcoord1.xy * _DecalTex_ST.xy + _DecalTex_ST.zw;
                tmp1 = tex2D(_DecalTex, tmp1.xy);
                tmp1.xyz = tmp1.xyz - tmp0.xyz;
                tmp0.xyz = tmp1.www * tmp1.xyz + tmp0.xyz;
                tmp0.xyz = tmp0.xyz * _Color.xyz;
                tmp1.xy = inp.texcoord.xy * _SpecTex_ST.xy + _SpecTex_ST.zw;
                tmp1 = tex2D(_SpecTex, tmp1.xy);
                ...
                tmp1.xyz = _EmissionColor.xyz * float3(0.305306, 0.305306, 0.305306) + float3(0.6821711, 0.6821711, 0.6821711);
                tmp1.xyz = _EmissionColor.xyz * tmp1.xyz + float3(0.0125229, 0.0125229, 0.0125229);
                tmp1.xyz = tmp1.xyz * _EmissionColor.xyz;
                tmp1.w = unity_UseLinearSpace != 0.0;
                tmp1.xyz = tmp1.www ? _EmissionColor.xyz : tmp1.xyz;
